...
data_js = (ROOT / "lineaige-data.js").read_text(encoding="utf-8") if (ROOT / "lineaige-data.js").exists() else ""
...
static_event_ids = re.findall(r'data-event="([^"]+)"', index)
if static_event_ids:
    require(index, 'data-event="now" aria-label="Enter the living present"', "static transition renderer must keep NOW keyboard-addressable and labelled")
else:
    require(index, 'id="lineaigeTimeline"', "data-driven renderer requires a canonical timeline mount")
    require(index, '<script src="lineaige-data.js"></script>', "data-driven renderer must load the canonical data adapter")
...
for match in re.finditer(r'<button\b([^>]*)>', index):
    ...
    if 'data-event=' in attrs and 'aria-label=' not in attrs:
        fail("every static timeline data-event button requires an aria-label")
...
require(js, "lastTrigger.focus", "closing the record detail must restore focus to the triggering timeline control")
...
require(js, "function safeExternalUrl", "external canonical source URLs must be protocol-checked before rendering")
require(js, "RELATIONSHIP UNAVAILABLE", "broken traversal targets must fail visibly rather than silently")
require(js, "No relationship claim is inferred from a broken path.", "broken traversal copy must explicitly avoid inferred causality")
require(js, "source.sourceRole", "source evidence roles must be exposed in the reader-facing source trail")
require(js, "geography.country", "structured geographic context must be surfaced when present")
...
print("OK: frontend accessibility, responsive, graceful-failure integration, evidence-role rendering, and theme-neutral traversal checks passed")
